#!/usr/bin/env python3
"""
Decodes current_display.png via ffmpeg and writes it directly to the
P5Large pixel overlay buffer, holding the image for DURATION seconds.
Called from the 'breaking_news' FPP Script playlist entry.

Pixel data starts at byte offset 12 in FPP-Model-Overlay-Buffer-P5Large:
...
# 1. Enable P5Large and force-create the shared memory overlay buffer
fpp_cmd('Overlay Model State', [MODEL, 'Enabled'])
fpp_put(f'/api/overlays/model/{MODEL}/mmap')
time.sleep(0.25)

# 2. Decode PNG -> raw RGB24 using ffmpeg (192x192 = 110592 bytes)
proc = subprocess.run(
    ['ffmpeg', '-i', IMAGE,
...
# 4. Hold for DURATION seconds; channelData retains image (nothing resets it).
#    Ticker text runs independently on TickerZone during this time.
time.sleep(DURATION)

# 5. Disable P5Large so the normal show resumes cleanly after playlist ends.
